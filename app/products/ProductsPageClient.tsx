...
export default function ProductsPageClient() {
  ...
  const partnersArray = useMemo(
    () =>
      partnersParam
        ? partnersParam.split(",").map((s) => s.trim()).filter(Boolean)
        : [],
    [partnersParam]
  );
    ...
    return products
      .filter((p) => {
        ...
        if (partnersArray.length > 0) {
          ...
          const brandOk = p.brand
            ? lowerPartners.includes(p.brand.toLowerCase())
            : false;
  ...
  const finalList = useMemo(() => {
    ...
    if (sortParam === "price-asc" || sortParam === "price-ascending") {
      arr.sort((a, b) => {
        const pa =
          (a.priceNow ??
