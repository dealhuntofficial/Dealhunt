...
export async function GET(req: Request) {
  try {
    ...
    if (merchant) items = items.filter((d) => d.merchant?.toLowerCase().includes(merchant.toLowerCase()));
    if (q) items = items.filter((d) => d.title.toLowerCase().includes(q.toLowerCase()));
    ...
    else if (sort === "discount-desc") items.sort((a, b) => (b.discount || 0) - (a.discount || 0));
